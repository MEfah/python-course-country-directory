    ...
    async def render(self) -> tuple[str, ...]:
        ...
        await tb.add_section(
            *[
                ...
                for language in self.location_info.location.languages
            ]
        )
        ...
        if len(self.location_info.news) > 0:
            await tb.add_header("НОВОСТИ")
            await tb.add_section(*[news.title for news in self.location_info.news])
...
class TableBuilder:
    ...
    async def add_section(self, *lines: str) -> None:
        """
    ...
    async def add_header(self, text: str) -> None:
        """
    ...
    async def _add_border_tline(self) -> None:
        """
        Добавить горизонтальную строку таблицы
        """
        self.table_lines.append("╠" + "═" * (self.table_size - 2) + "╣")

    async def _add_content_tline(self, content: str, center: bool = False) -> None:
        """
        ...
        if center:
            ...
            for line in content_lines:
                self.table_lines.append(
                    "║ " + line + " " * (self.table_size - 4 - len(line)) + " ║"
                )
